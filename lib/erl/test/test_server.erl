...
-export([go/0, go/1, start_link/2, handle_function/3]).
...
handle_function(_ServiceName, testVoid, {}) ->
    io:format("testVoid~n"),
    ok;

handle_function(_ServiceName, testString, {S}) when is_binary(S) ->
    io:format("testString: ~p~n", [S]),
    {reply, S};

handle_function(_ServiceName, testByte, {I8}) when is_integer(I8) ->
    io:format("testByte: ~p~n", [I8]),
    {reply, I8};

handle_function(_ServiceName, testI32, {I32}) when is_integer(I32) ->
    io:format("testI32: ~p~n", [I32]),
    {reply, I32};

handle_function(_ServiceName, testI64, {I64}) when is_integer(I64) ->
    io:format("testI64: ~p~n", [I64]),
    {reply, I64};

handle_function(_ServiceName, testDouble, {Double}) when is_float(Double) ->
...
handle_function(_ServiceName, testMap, {Map}) ->
    io:format("testMap: ~p~n", [dict:to_list(Map)]),
    {reply, Map};

handle_function(_ServiceName, testSet, {Set}) ->
    true = sets:is_set(Set),
    io:format("testSet: ~p~n", [sets:to_list(Set)]),
    {reply, Set};

handle_function(_ServiceName, testList, {List}) when is_list(List) ->
    io:format("testList: ~p~n", [List]),
    {reply, List};

handle_function(_ServiceName, testEnum, {Enum}) when is_integer(Enum) ->
    io:format("testEnum: ~p~n", [Enum]),
    {reply, Enum};

handle_function(_ServiceName, testTypedef, {UserID}) when is_integer(UserID) ->
    io:format("testTypedef: ~p~n", [UserID]),
    {reply, UserID};

handle_function(_ServiceName, testMapMap, {Hello}) ->
...
handle_function(_ServiceName, testInsanity, {Insanity}) when is_record(Insanity, insanity) ->
...
handle_function(_ServiceName, testMulti, Args = {Arg0, Arg1, Arg2, _Arg3, Arg4, Arg5})
...
handle_function(_ServiceName, testException, {String}) when is_binary(String) ->
...
handle_function(_ServiceName, testMultiException, {Arg0, Arg1}) ->
...
handle_function(_ServiceName, testOneway, {Seconds}) ->
